...
Status FunctionToFlatFunctionDefBuffer(FunctionDef fdef, TF_Buffer* out) {
  LOG(ERROR) << "== FunctionToFlatFunctionDefBuffer: START";
  ...
  for (const NodeDef& node : fdef.node_def()) {
    ...
    *reinterpret_cast<size_t*>(nodes_buf_it) = node_size;
    ...
    if (!node.SerializeWithCachedSizesToArray(nodes_buf_it)) {
      LOG(ERROR)
          << "== FunctionToFlatFunctionDefBuffer: Unable to serialize pb";
  ...
  if (tf_buf == nullptr) {
    LOG(ERROR)
        << "== FunctionToFlatFunctionDefBuffer: Failed to allocate memory";
  ...
  *reinterpret_cast<size_t*>(tf_buf) = func_info_size;
  if (!fdef.SerializeWithCachedSizesToArray(tf_buf + sizeof(size_t))) {
    port::Free(tf_buf);
    LOG(ERROR) << "== FunctionToFlatFunctionDefBuffer: Unable to serialize pb";
  ...
  LOG(ERROR) << "== FunctionToFlatFunctionDefBuffer: END";
  return OkStatus();
}
